 ...
 * @returns  The path relative to source directory.
 */
const relative = (p: string) => path.join(__dirname, "../src", p);
...
const favs = fs.readFileSync(relative("favs.json"), "utf8");
try {
...
app.get("/", (_, res) => {
  return res.sendFile(relative("index.html"));
});
